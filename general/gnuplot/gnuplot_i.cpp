...
Gnuplot& Gnuplot::savetotex(const std::string filename){
    std::ostringstream cmdstr;
    cmdstr << "set terminal epslatex size 3.321,2.485 standalone color colortext 8";
    cmd(cmdstr.str() );
    ...
    cmdstr.str("");     // Clear cmdstr
    cmdstr << "set style line 1 lc rgb \'#0060ad\' lt 1 lw 2 pt 7 ps 1.2";
    cmd(cmdstr.str());

    cmdstr.str("");     // Clear cmdstr
    cmdstr << "set style line 2 lc rgb '#dd181f' lt 1 lw 2 pt 5 ps 1.2";
    cmd(cmdstr.str());

    return *this;
}
// convert generated tex file to eps
Gnuplot& Gnuplot::outputpdf(const std::string filename){
    reset_all();
    std::ostringstream cmdstr;
    cmdstr << "latex -interaction=batchmode " << filename << ".tex >/dev/null;"<<
    " dvips -q -o "<< filename <<".eps "<< filename <<".dvi; "<<
    "rm "<< filename <<"-inc.eps; rm "<< filename <<".aux; rm "<< filename <<
    ".dvi; rm "<< filename <<".log; rm "<< filename <<".tex; "
    <<"ps2pdf "<< filename<<".eps "<<filename<<".pdf; ";
    #ifndef __APPLE__
    cmdstr<<"evince "<< filename<<".pdf &";
    #else
    cmdstr<<"open "<< filename<<".pdf";
    #endif
