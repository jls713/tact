...
class NFW_triax(object):
    def __init__(self,GM,rs,q_1,q_2):
        self.GM=GM
        self.rs=rs
        self.q1=q_1*q_1
        self.q2=q_2*q_2
    def pot(self,x,y,z):
        r = np.sqrt(x*x+y*y/self.q1+z*z/self.q2)
        return -self.GM*np.log(1.+r/self.rs)/r
    ...
    def tot_force(self,x,y,z):
        r = np.sqrt(x*x+y*y/self.q1+z*z/self.q2)
        dpdr = self.GM*(np.log(1.+r/self.rs)/r-1./self.rs/(1.+r/self.rs))/r
        return np.array([-dpdr*x/r, -dpdr*y/r/self.q1, -dpdr*z/r/self.q2])
