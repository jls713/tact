...
def plot3D_stacktriax(initial,final_t,N_MAT,file_output):
    ...
    a[0,2].annotate('',(arrow_end+arrowd,act[1]),(arrow_end,act[1]),arrowprops=dict(arrowstyle='<-',color='b'),annotation_clip=False)
    a[0,2].annotate('',(arrow_end+arrowd,act[2]),(arrow_end,act[2]),arrowprops=dict(arrowstyle='<-',color='r'),annotation_clip=False)
    # True actions
...
def triaxialfudgepaper():
    TT = pot.NFW_triax(12.3e5,10.,0.95,0.85)
    E = TT.H(np.array([0.,10.,0.]))
    for y in np.arange(1.3234,10.,0.5):
        for th in np.arange(0.1,np.pi/2.,0.1):
            p = np.sqrt(2.*E-2.*TT.pot(0.01,y,0.01))
            initial = np.array([0.01,y,0.01,p*np.cos(th),0.05,p*np.sin(th)])
            timeseries=np.linspace(0.,10.5,10.5/0.005)
            results = odeint(pot.orbit_derivs2,initial,timeseries,args=(TT,),rtol=1e-13,atol=1e-13)
            (act,ang,n_vec,toy_aa, pars),loop = find_actions(results, timeseries,N_matrix=6,ifloop=True,ifprint=False)
            print y,th,act[0],act[1],act[2]
...
if __name__=="__main__":
    ...
    # longa = plot3D_stacktriax(LongP,8*LowestPeriodLong,6,'genfunc_3d_example_LT_Stack_long.pdf')
