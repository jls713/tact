...
def solver(AA, N_max, symNx = 2, throw_out_modes=False):
    ...
    # plt.figure(figsize=[3.32,3.32/1.6])
    # LR = lr(angs[:500].T[0],angs[:500].T[1])
    # LR2 = lr(angs[-500:].T[0],angs[-500:].T[1])
    # plt.xlabel(r'$\theta_1/\pi$')
    # plt.ylabel(r'$\theta_2/\pi$')
    # plt.plot(angs.T[0]/np.pi,(LR[0]*angs.T[0]+LR[1])/np.pi,'r')
    # plt.plot(angs.T[0]/np.pi,(LR2[0]*angs.T[0]+LR2[1])/np.pi,'b')
    # plt.plot(angs.T[0]/np.pi,angs.T[1]/np.pi,'k.',markersize=0.8)
    # plt.savefig("../../Documents/thesis/genfunc_aa/nonchaotic_unrolled_angs.pdf",bbox_inches='tight')
    # plt.clf()
    ...
    check_each_direction(n_vectors,angs,ifprint=False)
