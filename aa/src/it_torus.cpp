...
VecDoub IterativeTorusMachine::RefineGuess(VecDoub PSP, VecDoub aa, Potential *Phi, double *min, Angles &theta_new, double dJ){
	// Construct torus
	int flag,err=0;
	Actions  J,oJ;	Angles theta;

	// Perhaps better to keep reusing same torus??
	Torus    *T; 	T = new Torus;
...
VecDoub IterativeTorusMachine::actions(const VecDoub &X, void*params){
	...
	while(min>eta and iter<MaxIterations){
		...
		VecDoub xNEW = RefineGuess(x, aaGuess, Phi->PWD(), &min, theta,dJ);
	...
	flag = T->AutoFit(J,Phi->PWD(),dJ,700,300,15,5,24,200,24,err);
	...
	return {aaGuess[0],aaGuess[1],aaGuess[2], // Action results
			...
			theta[0],theta[1],theta[2],		  // Angle results
